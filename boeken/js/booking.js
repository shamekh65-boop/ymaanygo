...
function setRideFor(v){
  ...
  document.getElementById('forMeBtn')
    .classList.toggle('active', v === 'me');

  document.getElementById('forOtherBtn')
    .classList.toggle('active', v === 'other');

  document.getElementById('otherPersonBlock')
    .classList.toggle('hidden', v !== 'other');
}
...
function setMinTime(){
  ...
  document.getElementById('when').min =
    toLocalValue(now);
...
function addStop(){
  ...
  const wrap =
    document.getElementById('stopsWrap');
...
function removeStop(id){
  ...
  const i =
    stops.ids.indexOf(id);

  if(i >= 0)
    stops.ids.splice(i, 1);
...
async function routeKm(A, B){
  ...
  const url =
    `${API.OSRM}/route/v1/driving/${A.lon},${A.lat};${B.lon},${B.lat}?overview=false`;
...
async function routeMulti(points){
  ...
  for(let i = 0; i < points.length - 1; i++){
    ...
    const r =
      await routeKm(points[i], points[i + 1]);
