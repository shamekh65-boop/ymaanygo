...
const WHATSAPP_NUMBER = "31633337184";
let calcTimer = null;
...
async function calc(force){
  ...
  for(const addr of addresses){
    ...
    if(!p){
      ...
      if(force){
        alert(
          getLang() === 'ar'
            ? "عنوان غير واضح. استخدم 📍 وحدد الدبوس من الخريطة."
            : (getLang() === 'en'
              ? "Unclear address. Use 📍 and pick from map."
              : "Adres onduidelijk. Gebruik 📍 en kies via kaart.")
        );
      }
      return;
    }
  ...
  if(car === "Comfort") price += pricing.COMFORT;

  document.getElementById('price').textContent = "€ " + price.toFixed(2);
  document.getElementById('meta').textContent =
    (getLang() === 'ar')
      ? `المسافة: ${r.km.toFixed(1)} كم • المدة: ${Math.round(r.min)} دقيقة`
      : (getLang() === 'en')
        ? `Distance: ${r.km.toFixed(1)} km • Duration: ${Math.round(r.min)} min`
...
function sendWhatsApp(){
  ...
  if(!when || isPastPickup(when, 15)){
    alert(
      lang === 'ar'
        ? "اختر وقتًا صحيحًا (بعد 15 دقيقة على الأقل)."
        : (lang === 'en'
          ? "Pick a valid time (min 15 minutes from now)."
          : "Kies een geldig tijdstip (min 15 min vanaf nu).")
    );
    return;
  }
  ...
  if(rideFor === 'other'){
    const on = (document.getElementById('otherName').value || "").trim();
    const op = (document.getElementById('otherPhone').value || "").trim();
      ...
      alert(
        lang === 'ar'
          ...
          : (lang === 'en'
            ...
            : "Vul naam en telefoon in.")
      );
      return;
    }

    if(!isValidNLPhone(op)){
      alert(
        lang === 'ar'
          ? "رقم الهاتف يجب أن يكون 06xxxxxxxx أو +31xxxxxxxxx."
          : (lang === 'en'
            ? "Phone must be 06xxxxxxxx or +31xxxxxxxxx."
            : "Telefoon: 06xxxxxxxx of +31xxxxxxxxx.")
      );
      return;
    }
  ...
  const msg =
    (lang === 'ar'
      ? `حجز تاكسي جديد:
من: ${from}
توقفات: ${stopVals.length ? stopVals.join(" -> ") : "لا يوجد"}
إلى: ${to}
وقت الالتقاط: ${when}
السيارة: ${car}
الركاب: ${pax}
...
${passengerLine}`
      : (lang === 'en'
        ? `New taxi booking:
From: ${from}
Stops: ${stopVals.length ? stopVals.join(" -> ") : "None"}
To: ${to}
Pickup time: ${when}
Car: ${car}
Passengers: ${pax}
...
${passengerLine}`
        : `Nieuwe taxi boeking:
Van: ${from}
Tussenstops: ${stopVals.length ? stopVals.join(" -> ") : "Geen"}
Naar: ${to}
Ophaalmoment: ${when}
Voertuig: ${car}
Passagiers: ${pax}
